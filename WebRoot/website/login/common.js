...
(function($) {
	...
	$.checkLogin = function() {
		var result = false;
		$.ajax({
			url: shopxx.base + "/web/checkLogin",
			type: "GET",
			dataType: "json",
			cache: false,
			async: false,
	...
	$.redirectLogin = function (redirectUrl, message) {
		var href = shopxx.base + "Project/website/";
		if (redirectUrl != null) {
			href +=  encodeURIComponent(redirectUrl);
		}
		if (message != null) {
	...
	$.redirectRegister = function (redirectUrl, message) {
		var href = shopxx.base + "Product/website/";
		if (redirectUrl != null) {
			href += encodeURIComponent(redirectUrl);
		}
		if (message != null) {
	...
	$(document).ajaxComplete(function(event, request, settings) {
		...
		}else if (loginStatus == "wapAccessDenied"){
			$.redirectWapLogin(location.href, "请登录后再进行操作");
...
function over(){ 
	...
	$.ajax({
		...
		cache:false,
		success: function(data) {
			$amount=$(data).find("#effectivePrice").text();
			$delete=$(data).find("a.delete");
			...
			if($amount!=""){	
				...
				$("#cartlist").append('<div class="autoDiv"><div style="text-align:left;width:160px;" class="left"><span>共计<i>&nbsp;'+count+'&nbsp;</i>件商品</span><br/><b>合计:</b><label>'+$amount+'</label></div><div class="right"><a href="javascript:;" class="check_out_btn right" style="color:#fff;" onclick="location=\'/cart/list.jhtml\'">去结算</a></div>');
